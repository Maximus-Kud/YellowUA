...
import defaultIcon from '../../assets/products/default-product.png'
...
function ProductLayout(props: Props) {
  ...
  return (
    <div className="product-layout">
      <span className="path">
        <span>Головна</span>
        <svg height="800px" width="800px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" enableBackground="new 0 0 50 50">
          ...
          <div id='filterModel' className='filter'>
            ...
            {filterModel.map(model => (
              <div className='filterTextWithCheckbox' key={model.name}>
                <input type='checkbox' defaultChecked={model.name === "iPhone 17 Pro Max"} />
            ...
            <div key={product.id} className='item'>
              <div className='top'>
                <div className='images'>
                  ...
                  <img src={product.image || defaultIcon} alt={product.name} className='photo'></img>
                </div>
              ...
              <div className='bottom'>
                ...
                <div className='buttons'>
                  <button className={product.isAvailable ? "buyButton" : "buyButton order"}>{product.isAvailable ? "Купити" : "Замовлення"}</button>
                  <button className={product.isAvailable ? "creditButton" : "hidden"}>В кредит</button>
                </div>
                <div className='description'><div className='status'>{product.isAvailable ? "Є в наявності" : "Немає в наявності"}</div><div className='code'>код: {product.code}</div></div>
              </div>
            </div>
          ))}
        </div>
